...
function App(){
 ...
 const [health,setHealth]=useState<Health|null>(null),[input,setInput]=useState(''),[provider,setProvider]=useState('gemini'),[mode,setMode]=useState('answer');
 ...
 return <div className={'app '+(artifact?'with-artifact':'')}>
  ...
  <aside className={'sidebar '+(sidebar?'open':'')}>
   ...
   <button className="system-button" onClick={()=>{setSystem(!system);refresh()}}><span className={'dot '+(health?.ready?'good':'')}/><span>{health?.ready?'Workspace ready':'Check system status'}</span><ChevronRight size={14}/></button>
   <div className="profile"><span>Y</span><div>Your workspace<small>Personal · Cloud deployment</small></div></div>
  </aside>
  <main className="main">
   <header className="topbar"><div className="breadcrumb"><button className="icon-button mobile-menu" aria-label="Open conversations" onClick={()=>setSidebar(true)}><Menu size={20}/></button><span>Workspace</span><ChevronRight size={13}/><strong>{messages.length?'Conversation':'New conversation'}</strong></div><div className="provider"><span className="dot good"/><select aria-label="Model provider" value={provider} disabled={busy} onChange={e=>setProvider(e.target.value)}><option value="gemini">Gemini · Cloud</option><option value="ollama">Ollama · Local</option><option value="anthropic">Claude · Cloud</option></select></div></header>
   {system&&<section className="system-panel"><div><strong>System status</strong><button className="icon-button" aria-label="Close system status" onClick={()=>setSystem(false)}><X size={17}/></button></div><dl>{[['PostgreSQL',health?.database?'Connected':'Unavailable'],['Transcript index',`${health?.episodes||0} episodes · ${health?.chunks||0} passages`],['Gemini',health?.gemini_configured?'Configured':'Not configured'],['Pi agent',health?.agent?'Connected':'Unavailable'],['Search',health?.embedding_model_ready?'Ready':'Unavailable']].map(([k,v])=><React.Fragment key={k}><dt>{k}</dt><dd>{v}</dd></React.Fragment>)}</dl><p>This public workspace uses Gemini and the hosted transcript index. Check the connection details above if a service is unavailable.</p></section>}
   ...
   <div className="composer-area">{error&&<div role="alert" className="notice error">{error}<button aria-label="Dismiss error" onClick={()=>setError('')}><X size={15}/></button></div>}{warning&&<div className="notice warning">{warning}</div>}{busy&&<div className="live-status" role="status"><span className="pulse"/>{status}</div>}
    ...
    <div className="composer-note"><ShieldCheck size={12}/>{provider==='ollama'?'Local inference. Your questions stay on this machine.':provider==='gemini'?'Cloud mode. Retrieved excerpts and conversation context are sent to Google Gemini.':'Cloud mode. Retrieved excerpts and conversation context are sent to Anthropic.'}<span>Verify important claims.</span></div>
   </div>
  </main>
